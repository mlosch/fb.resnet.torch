...
local function createModel(opt)
   ...
   local function lockedscalepyramid(inFeatures, outFeatures, ksizes, strides, paddings)
     ...
     for i=k,1,-1 do
        ...
        node:add(Convolution(inFeatures, outFeatures,
        ksizes[i], ksizes[i],
        strides[i], strides[i],
     ...
     return layer --out: b x 4 x nf x H x W
   end

   local function minblock(nfeatures, nscales, enable_pooling)
     enable_pooling = enable_pooling or false

     local minlayer = nn.Sequential()
     minlayer:add(nn.Transpose({1,3})) --out: nf x 4 x b x H x W

     -- ParametricMin expects: nf x 4 x b x H x W
     minlayer:add(nn.ParametricMin(nscales, false))
     minlayer:add(nn.Sum(2))
     minlayer:add(nn.Transpose({1,2})) --back to: b x nf x H x W
     minlayer:add(SBatchNorm(nfeatures))
     if enable_pooling then
        minlayer:add(Max(3,3,2,2,1,1))
     end
     minlayer:add(ReLU(true))
   ...
   if opt.dataset == 'imagenet' or opt.dataset == 'lmdb' then
      ...
      model:add(lockedscalepyramid(3,64,{3,5,7,9},{2,2,2,2},{0,1,2,3}))
   ...
   local function CConvInit()
     local modules = model:findModules('cudnn.SpatialConvolutionT')
     for i=#modules,1,-1 do
       local v = modules[i]
       local n = v.kW*v.kH*v.nOutputPlane

       if i < #modules then
         v.masterFilter = modules[#modules].weight
         v:reset()
       else
         v.weight:normal(0,math.sqrt(2/n))
       end
